...
var configExtractorAllowlist = map[string]string{
	...
	"aws_kendra":            "[no-inspector] Kendra inspector deferred (#760 — the kendra service is not registered in AWSServiceActions, so no discovery-inspector handler or AWS/Kendra namespace group exists yet; the preset ships no observability alarm and no SDK-shape extractor; index + S3 data source are surfaced via name-prefix scoping until a kendra inspector lands; mirrors the aws_agentcore_gateway precedent)",
