...
var configExtractorAllowlist = map[string]string{
	...
	"aws_agentcore_gateway": "[no-inspector] AgentCore Gateway SDK-shape extractor deferred (#763 — the agentcore discovery dispatcher + AWS/Bedrock-AgentCore metrics namespace group + SystemErrors alarm now ship, and ComponentMetricsMapping binds agentcore.list-gateways; what remains deferred is a per-component SDK-shape extractor reading gateway + target detail fields for the panel config map — surfaced via name-prefix scoping until it lands. The reverse-import discovery vocabulary for aws_bedrockagentcore_gateway/_target is a separate concern from the metrics panel; mirrors the aws_kendra precedent)",
	"aws_kendra":            "[no-inspector] Kendra SDK-shape extractor deferred (#760 — the kendra discovery dispatcher + AWS/Kendra metrics namespace group + DocumentsFailedToIndex alarm now ship, and ComponentMetricsMapping binds kendra.list-indices; what remains deferred is a per-component SDK-shape extractor reading index/data-source detail fields for the panel config map — surfaced via name-prefix scoping until it lands. The reverse-import extractor is a separate concern from the metrics panel, #2113)",
