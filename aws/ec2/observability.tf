...
#
# Co-located alarm authoring for the EC2 instance this module owns.
# Net-new alarm; the legacy aggregator-side ec2_cpu_high alarm is wired
# only with bastion instance IDs today (pkg/composer/contracts.go:697),
# so EC2 has no legacy alarm to relocate.
...
locals {
  ...
  _obs_runbook = var.runbook_url_prefix == "" ? "" : " Runbook: ${var.runbook_url_prefix}/ec2/cpu"
}

resource "aws_cloudwatch_metric_alarm" "cpu_high" {
  for_each = var.enable_observability ? { "0" = true } : {}

  alarm_name          = "${module.name.name}-ec2-cpu"
  ...
  alarm_description   = "EC2 instance CPU above ${local._obs_thresholds["cpu_high_pct"]}%.${local._obs_runbook}"
