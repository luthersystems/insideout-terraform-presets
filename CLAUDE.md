...
These preset files are embedded at build time into the InsideOut backend via Go's `embed.FS`. The composition engine does:
...
7. The composed stack is tar.gz'd and deployed by the InsideOut deployment service
...
- **Project tag is required on every taggable AWS resource.** Use `tags = merge(module.name.tags, var.tags)` so the `Project` tag emitted by `module.name.tags` reaches the resource. The downstream InsideOut inspector filters on exact `Project = <project>` match, so untagged resources are invisible to drift detection and CloudWatch metrics (see issue #81). If a resource accepts any tag-shaped attribute (including listeners, instance profiles, and IAM roles/policies in provider 5.x+), tag it.
- **GCP mirror:** every labelable GCP resource must set `labels = merge({ project = var.project }, var.labels)` (or equivalent) so project identity propagates. Enforced in CI by `tests/lint-project-label.sh` using an allowlist of label-capable resource types (see script header for how to extend). For label-less GCP resource types (KMS keyrings, Firestore databases, alert policies, etc.) the parallel rule is **name-prefix scoping**: the resource's `name` / `account_id` / `display_name` must contain `${var.project}` so the InsideOut inspector can attribute it via the project-scoped API path + name pattern (see issue #215 comment-4364048339). Enforced in CI by `tests/lint-labelless-name-prefix.sh` with an allowlist + exempt-list (see script header for the exemption rationale per resource type).
- **GCP `var.project` vs `var.project_id` (issue #157):** these are two different things — never conflate them. `var.project` is the stack naming/label prefix and may legally hold values like `"io-abc123"` that aren't valid GCP project IDs. `var.project_id` is the real GCP project ID where resources are created (e.g. `"my-prod-12345"`). Every GCP module that creates resources in a project must declare both, and every `project = ...` argument on a `google_*` resource — and every vendored sub-module's `project_id = ...` — must reference `var.project_id`, never `var.project`. The `${var.project_id}.svc.id.goog` workload identity pool name in `gcp/gke` is a real-project-ID use too. Resource name interpolations (`name = "${var.project}-..."`) and label values (`labels = merge({ project = var.project }, var.labels)`) keep using `var.project` so the lint script and InsideOut inspector grouping continue to work. The composer surfaces `gcp_project_id_required` / `gcp_invalid_project_id` ValidationIssues at compose time, so callers see the misconfiguration before deployment is invoked. Callers populate the new field via `ComposeStackOpts.GCPProjectID` (and `ComposeSingleOpts.GCPProjectID`). Enforced in CI by `tests/lint-gcp-project-pin.sh` (#287, paired with reliable#1328 + ui-core#153 WIF support), which walks every `google_*` resource and `module` block under `gcp/**/main.tf` / `observability.tf` and fails if `project = var.project_id` (or `project_id = var.project_id` for modules) is missing — defending against the Google provider falling back to the credential JSON's `project_id` field, which for WIF / `external_account` JSONs is the WIF *pool* project rather than the user's stack project. Six parent-scoped resource families (e.g. `google_secret_manager_secret_version`, `google_storage_bucket_object`, `google_kms_crypto_key`) are listed in `EXEMPT_PROJECT_PIN_GCP` because the Google provider does not accept a `project` attribute on them; extending the skiplist requires verifying the resource has no `project` field in the provider schema.
...
- **Nested-object literal drift gate (issues #839, #844):** every generated typed model (`pkg/composer/imported/generated`) is cross-checked in CI against a real `terraform providers schema -json` dump at the composer's pinned provider version. The triggering example was `aws_route_table.route` missing the provider-Required `odb_network_arn` sub-attribute (#839): a nested-object attribute the emitter renders as an object literal (`attr = [{...}]`) is type-checked by terraform against the provider's FULL nested schema, so any Required sub-attr absent from our struct fails `terraform plan` the moment the provider adds it. The gate (`tests/drift-scan.sh`, `drift-scan` CI job) reads the pin from the single source of truth by composing a minimal `aws_s3` stack via `cmd/composetest` — the composed `providers.tf` carries the exact `= 6.52.0` pin from `pkg/composer/imported/provider_pins.go` — then runs `terraform init` + `providers schema -json` and diffs with `cmd/driftscan -fail-on-bugs`. Known-stale, issue-tracked misses live in `tests/driftscan-allowlist.txt` (print as `ALLOWED(stale-model)`); the allowlist can only shrink — each entry MUST cite its tracking issue (`aws_route_table.route:odb_network_arn  # refs #845`) or driftscan hard-errors, and rows are deleted when the model regenerates (#845).
- **Root-only blocks are forbidden in presets (issue #199):** Terraform 1.5+ permits `import {}` and `removed {}` blocks **only in the root module**. Every preset in this repo is consumed as a *child* module by the InsideOut composer (which emits `module "<key>" { source = "..." }` in a generated root), so any `import {}` or `removed {}` block placed in `aws/<m>/*.tf` or `gcp/<m>/*.tf` will fail `terraform init` with `An import block was detected in "module.<name>". Import blocks are only allowed in the root module.` This regressed v0.7.0 (#199) — standalone `terraform validate` accepted the block because the preset was treated as the root, but the bundle failed init at customer deploy time. Enforced in CI by `tests/lint-no-root-only-blocks.sh` (static grep) and the `validate-presets-as-child` workflow job (wraps each preset and runs `terraform init`). The same rule applies to `provider {}` blocks with arguments — root-only since TF 0.13.
- **AWS imported-provider credential contract (issue #677):** the composer-emitted `provider "aws"` blocks (default, `us_east_1`, and `aws.imported`) assume the customer's cross-account role via a single canonical variable pair — **`bootstrap_role`** (the role ARN) and **`aws_external_id`** (the confused-deputy external ID). These names match the `sandbox-infrastructure-template` wrapper's own wrapper-owned root declarations, so there is exactly one name set across the whole pipeline. The declarations live in `/providers.tf` (alongside the default provider), not in a dedicated sibling file: direct (non-wrapper) archives ship `/providers.tf` and stay self-contained; in wrapper mode the wrapper drops the composer's `/providers.tf` via `PRESERVE_PATTERNS` and declares `bootstrap_role` / `aws_external_id` itself, so the surviving sibling alias files (`/providers-aliases.tf`, `/providers-imported.tf`) resolve against the wrapper's declarations with no duplicate-declaration error. The legacy `bootstrap_role_arn` / `external_id` names and the dedicated `/variables-imported.tf` sibling (a name-mismatch workaround from #630) are retired. Do not reintroduce either. Enforced in CI by `TestGenerateProvidersTF_AWSImportedCredentialContract`.
- **Idempotency contract (issues #197, #199):** every preset must support back-to-back `terraform destroy` → `terraform apply` cycles without manual intervention. Resources backed by cloud-API singletons that can't truly be deleted — Identity Platform, Firestore databases, KMS key rings, project-level service activations — must be modeled as **adoption**, not creation, because the provider's CREATE will reject the second apply with `400 INVALID_*` once the underlying GCP/AWS state survives the destroy. The adoption mechanism is a root-level `import { to = module.<name>.<resource> ... }` block — but per the rule above, the preset itself **cannot** declare it. The InsideOut composer must emit the import block alongside the module instantiation; until that lands, presets pin `lifecycle { ignore_changes = all }` and document that callers running against pre-existing singletons must `terraform import` out-of-band before the first apply. See `gcp/identity_platform/main.tf` for the current shape (CREATE + `ignore_changes = all`, header comment links #199). For race-prone resources whose CREATE is order-sensitive (e.g. GCP IAM eventual consistency before downstream resource validation), insert an explicit `time_sleep` between the binding and the dependent — see `gcp/cloud_build/main.tf::time_sleep.wait_iam_propagation` (90s covers ~p99 propagation; only fires on creation). Today's known singleton candidates: `gcp_identity_platform` (#197 mitigated, #199 composer-emit-import follow-up). Audit follow-up: `gcp_firestore` (singleton database — once created, the project's default database can't be removed; needs the same adoption treatment, blocked on the same composer change).
...
- **Module-package boundaries.** Terraform rejects `../` source traversal
  ...
  package`. The InsideOut composer is responsible for bundling shared
  modules into the same workspace as their consumers so the relative source
  path resolves within a single package. For local
